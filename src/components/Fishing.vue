<template>
  <div>
    <div class="ui grid">
      <div class="three wide column" v-for = "(f, index) in fishs1" :key="f" v-bind:class="face0 == index ? 'face' : 'back'" @click = "flip(index,0)">
        <img v-bind:class="fishs1[face0] == fishs2[face1] ? 'gold' : '' " :src="f"/>
      </div>
    </div>
    <div class="ui grid">
      <div class="three wide r column" v-for = "(f, index) in fishs2" :key="index" v-bind:class="face1 == index ? 'face' : 'back'" @click = "flip(index,1)">
        <img v-bind:class="fishs1[face0] == fishs2[face1] ? 'gold' : ''" :src="f"/>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    remove: function () {
      ...
      this.flipback()
      if (!this.fishs1.length && !this.fishs2.length) {
        this.win()
        this.reset()
      }
    },
